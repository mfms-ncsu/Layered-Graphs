...
def main():
    if len(sys.argv) != 2:
        print("Usage: channelDegrees.py SGF_FILE")
        exit(1)
...
def print_channel_statistics(channel):
    ...
    lower_degrees = [_node_dictionary[node][1] for node in _nodes_on_layer[lower_layer]]
    lower_degrees = [x for x in lower_degrees if x != 0]
    # get the down degrees of nodes on the upper layer
    upper_degrees = [_node_dictionary[node][2] for node in _nodes_on_layer[upper_layer]]
    upper_degrees = [x for x in upper_degrees if x != 0]
...
def discrepancy(list):
    ...
    median = working_list[int((len(working_list) - 1)/ 2)]
...
#  [Last modified: 2020 05 18 at 15:05:35 GMT]
